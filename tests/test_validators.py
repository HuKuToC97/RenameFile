...
class TestValidators(unittest.TestCase):
    def test_validate_inputs_success(self):
        self.assertEqual(validate_inputs("/valid/path", "{number}_{name_file}.txt", "1", "2"), (1, 2))

    def test_validate_inputs_invalid_folder(self):
        with self.assertRaises(ValueError):
            validate_inputs("/invalid/path", "{number}_{name_file}.txt", "1", "2")
    ...
    def test_validate_inputs_invalid_numbers(self):
        with self.assertRaises(ValueError):
            validate_inputs("/valid/path", "{number}_{name_file}.txt", "one", "two")
